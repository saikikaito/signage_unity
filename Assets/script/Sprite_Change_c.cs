    ...
    void Update()
    {
        ...
        if (!is_AfterNoon && !is_night)
        {
            ...
            if (!random_tanuki_seed_)
            {
                ...
                //random_val_ = 6;
                Debug.Log($"SpriteChange_cの乱数:{random_val_}");
            ...
            if (!is_tanuki_) spriterenderer2_alpha = 1.0f - fadein_time / fade_change_time; //家の中のたぬき抽選失敗
            if (is_tanuki_) spriterenderer3_alpha = 1.0f - fadein_time / fade_change_time; //家の中のたぬき抽選成功

            spriteRenderer.color = new(red, green, blue, spriterenderer_alpha);
            if (!is_tanuki_) spriteRenderer2.color = new(red2, green2, blue2, spriterenderer2_alpha); //家の中のたぬき抽選失敗
            if (is_tanuki_) spriteRenderer3.color = new(tanu_red, tanu_green, tanu_blue, spriterenderer3_alpha); //家の中のたぬき抽選成功
        ...
        if (is_night && time_count_night > Start_fade_time)
        {
            ...
            if (!is_tanuki_) spriterenderer2_alpha = 1.0f - fadeout_time / fade_change_time; //家の中のたぬき抽選失敗
            if (is_tanuki_) spriterenderer3_alpha = 1.0f - fadeout_time / fade_change_time; //家の中のたぬき抽選成功
